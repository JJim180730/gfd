...
export class I18n {
     ...
     * @param options 格式化选项
     */
    translatePlural(key, count, params = {}, options = {}) {
    ...
    interpolate(text, params) {
        if (!text || !params)
            return text;
        const regex = new RegExp(`${this.escapeRegExp(this.interpolationPrefix)}\\s*(.+?)\\s*${this.escapeRegExp(this.interpolationSuffix)}`, 'g');
     ...
     * 使用模板格式化日期
     */
    formatDateWithTemplate(date, format, locale) {
