 ...
 * @param options 格式化选项
 */
export function Translate(keyPrefix?: string, options: FormatOptions = {}) {
 ...
 * @param options 格式化选项
 */
export function TranslateParam(paramIndex: number, options: FormatOptions = {}) {
  return function (target: any, propertyKey: string | symbol, descriptor: PropertyDescriptor) {
    const originalMethod = descriptor.value;
...
export function TranslateDynamic(
  getKey: (result: any) => string,
  getParams?: (result: any) => Record<string, any>,
