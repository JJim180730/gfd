...
import { isFunction, isObject, isClass, isNil, isPromise } from '@gfd/utils';
...
export class Container {
  ...
  private instances: Map<InjectionToken, any> = new Map();
  private config: Required<ContainerConfig>;
  private static defaultConfig: Required<ContainerConfig> = {
    autoRegister: true,
  ...
  registerClass<T>(
    ...
    lifecycle: Lifecycle = this.config.defaultLifecycle
  ): void {
    this.register({
      provide: token,
      useClass: clazz,
  ...
  get<T>(token: InjectionToken<T>): T {
    ...
    if (!this.providers.has(token)) {
      ...
      if (this.config.autoRegister && isClass(token) && this.isInjectable(token)) {
        this.registerClass(token, token as new (...args: any[]) => T);
      } else {
   ...
   * @param provider 提供者
   */
  private resolveProvider<T>(provider: Provider<T>): T | Promise<T> {
    // 值提供者
   ...
   * @param clazz 类
   */
  private resolveClass<T>(clazz: new (...args: any[]) => T): T {
    // 获取构造函数参数类型
    ...
    const dependencies = paramTypes.map((paramType: any, index: number) => {
      ...
      try {
        ...
        throw new Error(`无法解析类 ${clazz.name} 的第 ${index} 个参数: ${(error as Error).message}`);
      }
    });
    ...
    for (const injection of injections) {
      if (injection.propertyKey) {
        try {
          ...
          if (!injection.options?.optional) {
            throw new Error(`无法注入属性 ${String(injection.propertyKey)}: ${(error as Error).message}`);
          }
