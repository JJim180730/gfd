...
export class DependencyGraph<T = any> {
   ...
   */
  getDependencies(nodeId: string, recursive: boolean = true): string[] {
    const node = this.nodes.get(nodeId);
    if (!node) return [];

    const dependencies = new Set<string>(node.dependencies);
   ...
   * @param recursive 是否递归获取，默认 true
   */
  getDependents(nodeId: string, recursive: boolean = true): string[] {
    const node = this.nodes.get(nodeId);
    if (!node) return [];

    const dependents = new Set<string>(node.dependents);
