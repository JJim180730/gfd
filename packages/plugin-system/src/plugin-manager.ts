...
export class PluginManager {
  ...
  private readonly pluginStatus: Map<string, PluginStatus> = new Map();
  private readonly pluginOptions: Map<string, PluginLoadOptions> = new Map();
   ...
   * @param options 加载选项
   */
  async install(plugin: Plugin, options: PluginLoadOptions = {}): Promise<void> {
    ...
    if (plugin.compatibleGFDVersion) {
      ...
      if (!satisfiesVersion(gfdVersion, plugin.compatibleGFDVersion)) {
        throw new PluginError(
          `插件不兼容当前GFD版本，需要版本 ${plugin.compatibleGFDVersion}，当前版本 ${gfdVersion}`,
          pluginName,
    ...
    this.pluginStatus.set(pluginName, 'installed' as PluginStatus);
   ...
   * @param options 加载选项
   */
  async installBatch(plugins: Plugin[], options: PluginLoadOptions = {}): Promise<void> {
  ...
  async initialize(pluginName: string): Promise<void> {
    ...
    try {
      ...
      this.pluginStatus.set(pluginName, 'initialized' as PluginStatus);
      this.gfd.logger.debug(`插件 ${pluginName} 初始化完成`);
    } catch (error) {
      this.pluginStatus.set(pluginName, 'error' as PluginStatus);
      throw new PluginError(
        `插件初始化失败: ${(error as Error).message}`,
        pluginName,
        'INITIALIZE_FAILED',
        error as Error
      );
  ...
  async start(pluginName: string): Promise<void> {
    ...
    try {
      ...
      this.pluginStatus.set(pluginName, 'running' as PluginStatus);
  ...
  async stop(pluginName: string): Promise<void> {
    ...
    try {
      ...
      this.pluginStatus.set(pluginName, 'stopped' as PluginStatus);
